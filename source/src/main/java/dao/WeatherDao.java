...
public class WeatherDao {
	public boolean check (WeatherDto w) {
		...
		try {
			...
			String sql = "select count(*)from weather WHERE day = ?";
			PreparedStatement pStmt = conn.prepareStatement(sql);
			
			pStmt.setString(1,  w.getDay());
	...
	public ArrayList<WeatherDto> select(WeatherDto tenki) {
		//接続状態、ログイン結果の初期値
		Connection conn = null;
